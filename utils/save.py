...
def save_agent(agent, path):
    ...
    agent.model.save_weights(path + "/model.weights.h5",)
    agent.target_model.save_weights(path + "/target_model.weights.h5",)
...
def load_agent(path, retrain = True, verbose = True):
    ...
    agent.model.load_weights(path + '/model.weights.h5')
    agent.model.build(agent.input_shape)   
    
    if retrain:
        agent.target_model = agent.build_model(trainable = False)
        agent.target_model.load_weights(path + '/target_model.weights.h5')
